...
public class SarachnisHelperOverlay extends OverlayPanel {

    private final SarachnisHelper plugin;
    private final SarachnisHelperConfig config;

    @Inject
    private SarachnisHelperOverlay(final SarachnisHelper plugin, final SarachnisHelperConfig config) {
        super(plugin);
        setPosition(OverlayPosition.ABOVE_CHATBOX_RIGHT);
        this.plugin = plugin;
        this.config = config;
        getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "SarachnisHelper"));
    }
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        /* Title and width */
        String title = "CS-SarachnisHelper";
