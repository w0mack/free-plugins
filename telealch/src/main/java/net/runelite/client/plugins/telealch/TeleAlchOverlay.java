...
public class TeleAlchOverlay extends OverlayPanel {

    private final tTeleAlch plugin;
    private final TeleAlchConfig config;

    @Inject
    private TeleAlchOverlay(final tTeleAlch plugin, final TeleAlchConfig config) {
        super(plugin);
        setPosition(OverlayPosition.ABOVE_CHATBOX_RIGHT);
        this.plugin = plugin;
        this.config = config;
        getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, plugin.getName()));
    }
